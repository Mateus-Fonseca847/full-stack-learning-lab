...
### 1.1 Como escrever um objeto?

**Object literal**
```javascript
const contabancaria = {
  nome: "Mateus de Souza Fonseca",
  saldo: 500,
  numeroDaConta: "2939243939",
  displayConta: function(){
    console.log(`A conta do proprietário ${this.nome}, de número ${this.numeroDaConta}, possui ${this.saldo} em saldo.`)
  },
}
```
É o formato de objeto escrito em linha, descrito por pares de chave e valor. Esse tipo de objeto pode deixar o código muito grande e não é muito prático para grandes operações

---

**Factory function**
```javascript
function createAccount(nome, numeroDaConta, saldoInicial) {
  return{
    nome: nome,
...
createAccount(Mateus, 2344252354, 200);

```
Ela funciona como uma “fábrica”: você passa informações para a função, e ela devolve um novo objeto já estruturado com essas informações.

---

**Constructor**:
```javascript
//Escrita sempre em PascalCase
function Account(nome, numeroDaConta, saldoInicial) {
    this.nome: nome,
    this.numeroDaConta: numeroDaConta,
    this.saldo: saldoInicial
}

const account1 = new Account(Mateus, 134325325, 300)

```

Constructor Function é uma função utilizada como modelo para a criação de objetos em JavaScript.
